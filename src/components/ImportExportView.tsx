...
import { listEntries, newEntryId, saveEntry } from '~/lib/journalDb'
...
export default function ImportExportView({ uid, mode }: Props) {
  ...
  const exportPdf = async () => {
    ...
    try {
      ...
      doc.setFontSize(20)
      doc.text('oriz Journal export', margin, y)
      y += 28
      ...
      for (const e of rows) {
        ...
        doc.setFont('helvetica', 'normal')
        doc.setFontSize(10)
        doc.text(`${e.entryDate} · ${e.journalType}${e.mood ? ` · ${e.mood}` : ''}`, margin, y)
        y += 14
  ...
  const importFile = async (file: File) => {
    ...
    try {
      ...
      if (file.name.endsWith('.json')) {
        ...
        const list = (Array.isArray(j) ? j : ((j.entries as unknown[]) ?? [])) as Record<
          string,
          unknown
        >[]
        for (const raw of list) {
          const date =
            (raw.creationDate as string)?.slice(0, 10) ||
            (raw.entryDate as string) ||
  ...
  if (mode === 'export') {
    return (
      <div className="ie">
        <p className="ie-lede">
          Take your data with you. Re-import any of these into another oriz journal.
        </p>
        <div className="ie-grid">
          <button type="button" disabled={busy} onClick={exportJson} className="ie-card">
            <strong>JSON dump</strong>
            <small>Lossless, includes every field.</small>
          </button>
          <button type="button" disabled={busy} onClick={exportMdZip} className="ie-card">
            <strong>Markdown ZIP</strong>
            <small>One .md per entry with front-matter.</small>
          </button>
          <button type="button" disabled={busy} onClick={exportMdBundle} className="ie-card">
            <strong>Single Markdown bundle</strong>
            <small>All entries concatenated.</small>
          </button>
          <button type="button" disabled={busy} onClick={exportPdf} className="ie-card">
            <strong>PDF</strong>
            <small>Print-friendly export of every entry.</small>
          </button>
        </div>
  ...
  return (
    <div className="ie">
      <p className="ie-lede">
        Drop in a Day One <code>.json</code>, a generic <code>.csv</code> (columns: title, body,
        mood, tags, date, type), or a single <code>.md</code> file. Each row becomes a new entry on
        your account.
      </p>
